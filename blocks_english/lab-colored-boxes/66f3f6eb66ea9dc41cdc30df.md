...
**注意：** 确保在 HTML 中链接你的样式表并应用你的 CSS。

# --hints--

`body` 应该有 `#f4f4f4` 的背景颜色。
...
你应该有一个 `div` 元素，其 class 为 `color-grid`。
...
这五个 `div` 元素中的每一个都应该有一个 `color-box` 和 `color#` 的 class——用 `div` 的顺序替换 `#` 符号。
...
`.color-box` 元素应该设置 `width` 和 `height`。
